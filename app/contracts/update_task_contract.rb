...
# Validates parameters for updating an existing Task.
#
# All fields are optional (partial update). Validates only what is provided.
class UpdateTaskContract < ApplicationContract
  ...
  rule(:starts_on, :recurring) do
    if values[:recurring] == true && values[:starts_on].nil? && key?(:starts_on)
