...
# Validates parameters for creating a new Task.
#
# Requires title; recurring flag determines which date fields are mandatory.
class CreateTaskContract < ApplicationContract
  ...
  params do
    ...
    required(:recurring).filled(:bool)
  ...
  rule(:title) do
    ...
    key.failure("is too long (maximum 255 characters)") if value.length > 255
  end
  ...
  rule(:due_date, :recurring) do
    if values[:recurring] == false && values[:due_date].nil?
