...
ActiveRecord::Schema[8.1].define(version: 2026_05_25_175401) do
  ...
  create_table "task_occurrences", force: :cascade do |t|
    ...
    t.check_constraint "status IS NULL OR (status::text = ANY (ARRAY['pending'::character varying, 'in_progress'::character varying, 'completed'::character varying, 'cancelled'::character varying]::text[]))", name: "chk_task_occurrences_status"
  end
  ...
  create_table "tasks", force: :cascade do |t|
    ...
    t.check_constraint "status::text = ANY (ARRAY['pending'::character varying, 'in_progress'::character varying, 'completed'::character varying, 'cancelled'::character varying]::text[])", name: "chk_tasks_status"
  end
